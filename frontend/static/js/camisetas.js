...
function applyFilters() {
  ...
  if (productCount) {
    productCount.textContent = `${visibleCount} produto${visibleCount !== 1 ? "s" : ""}`;
  }
...
function updateActiveFilters() {
  ...
  selectedColors.forEach((color) => {
    chips.push({ type: "color", value: color, label: `Cor: ${color}` });
  });

  selectedSizes.forEach((size) => {
    chips.push({ type: "size", value: size, label: `Tamanho: ${size}` });
  });
